...
int main()
{
	...
	//创建和编译Shader
	Shader ourShader("vertexshader.vert", "fragmentshader.frag", "geomshader.geom");

	Shader skyboxShader("skyboxvertex.vert", "skyboxfragment.frag");

	//加载模型
	Model ourModel("2B/2B.obj");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		ourShader.use();
		ourShader.setFloat("time", static_cast<float>(glfwGetTime())/20);
